...
const ACTIONS_NO_ID = new Set(['zombie-check']);

// Smart arg parsing: detect if first arg is an action (no pipeline ID needed)
let pipelineId, action, args;
const raw = process.argv.slice(2);
if (raw.length >= 1 && ACTIONS_NO_ID.has(raw[0])) {
  // e.g. "node pipeline-monitor.js zombie-check"
  pipelineId = null;
  action = raw[0];
  args = raw.slice(1);
} else {
  // e.g. "node pipeline-monitor.js <id> <action> [args...]"
  pipelineId = raw[0];
  action = raw[1];
  args = raw.slice(2);
}
...
if (!action) {
  ...
  console.error('       node pipeline-monitor.js zombie-check');
  process.exit(1);
}
